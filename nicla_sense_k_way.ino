...
#include <K-Way-Checkpoints_inferencing.h>
...
BLEStringCharacteristic pressureCharacteristic("5d75d723-fce9-471b-93e2-6c625ef6d634", BLERead | BLENotify, 10);
BLEStringCharacteristic gestureCharacteristic("5d75d724-fce9-471b-93e2-6c625ef6d634", BLERead | BLENotify, 10);
...
void setup()
{
    ...
    while (!Serial);
    Serial.println("K-way Checkpoint Demo\r\n");
    ...
    BLE.setLocalName("K-way-CP");
    // set the UUID for the service this peripheral advertises:
    BLE.setAdvertisedService(dataService); 
    // add the characteristics to the service
    dataService.addCharacteristic(pressureCharacteristic);
    dataService.addCharacteristic(gestureCharacteristic);
    pressureCharacteristic.writeValue("");
...
void loop()
{
    ...
    //strings for pressure and gesture
    //convert from pascals to in Hg for the US
    float pressInHg = baro.value() / 33.86;
    String pressureStr = String(pressInHg);
    String gesture = "";
    // print predictions
    ...
    ei_printf("Gesture result: %s, pressure reading: %s", gesture.c_str(), pressureStr.c_str());
    //send pressure and inference results to BLE
    if(central.connected()) {
      pressureCharacteristic.writeValue(pressureStr);
      //if the gesture indicates a checkpoint movement, send notification to app
      ei_printf("\nMax value: %f\n", maxValue);
      if(gesture == "checkpoint" && maxValue > 0.8)
      {
        ei_printf("\nSend checkpoint!\n");
        gestureCharacteristic.writeValue("checkpoint");
      }
    }
    else {
      nicla::leds.setColor(red);
